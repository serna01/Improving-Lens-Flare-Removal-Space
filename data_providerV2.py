...
def get_paired_scene_and_flare_dataset(scene_path,
                                       flare_path,
                                       source,
                                       batch_size,
                                       input_shape=(640, 640, 3),
                                       repeat=0):
    """Returns paired datasets of scene and flare images."""
    scene_dataset = get_scene_dataset(scene_path, source, batch_size, input_shape, repeat)
    flare_dataset = get_flare_dataset(flare_path, source, batch_size, input_shape, repeat)
    
    # Pair the datasets
    paired_dataset = tf.data.Dataset.zip((scene_dataset, flare_dataset))
